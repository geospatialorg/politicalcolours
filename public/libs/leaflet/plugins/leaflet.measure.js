L.Control.Measure = L.Control.extend({
	...
	onAdd: function (map) {
		...
		map.on('startMeasure', this._toggleMeasure, this);

		return container;
	},
	onRemove: function (map) {
		map.off('startMeasure', this._toggleMeasure, this);
	},
	...
	_toggleMeasure: function () {
		...
		if (this._measuring) {
			//add custom event
			this._map.fire('beginMeasure');
			L.DomUtil.addClass(this._container.childNodes[0], 'leaflet-activetoggle');
			this._startMeasuring();
		} else {
			L.DomUtil.removeClass(this._container.childNodes[0], 'leaflet-activetoggle');
	...
	_updateTooltipDistance: function (total, difference) {
		var totalRound, differenceRound, text;
		if (difference < 1000) {
			differenceRound = this._roundMeters(difference);
		} else {
			differenceRound = this._round(difference);
		}
		if (total < 1000) {
			totalRound = this._roundMeters(total);
			text = '<div class="leaflet-measure-tooltip-total">' + totalRound + ' m</div>';
		} else {
			totalRound = this._round(total);
			text = '<div class="leaflet-measure-tooltip-total">' + totalRound + ' km</div>';
		}

		if (differenceRound > 0 && difference < 1000 && total < 1000 && totalRound != differenceRound) {
	...
	_roundMeters: function (val) {
		return Math.round(val);
	},

	_round: function (val) {
		return Math.round((val / 1000) * 100) / 100;
		//return Math.round((val / 1852) * 100) / 100;
	},
