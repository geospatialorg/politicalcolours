...
$maptooltip.html('<h4>Chamber of deputies 2008-2012</h4>Hover over a uninominal college').show();
...
function enterLayer() {
    ...
    $maptooltip.html('<h4>Chamber of deputies 2008-2012</h4>' + judet + '</br>' + presedinte + '</br>' + party_colour + partid).show();
    this.bringToFront();
    this.setStyle({
        weight: 3,
        opacity: 1,
...
function leaveLayer() {
    //$maptooltip.hide();
    $maptooltip.html('<h4>Chamber of deputies 2008-2012</h4>Hover over a uninominal college');
    this.bringToBack();
    this.setStyle({
        weight: 1,
        opacity: 1,
...
function loadData() {
    $.ajax({
        type: "GET",
        url: site_url + "data/ro_deputati_2008.csv",
        dataType: "text",
        success: function (data) {
            ...
            $.getJSON(site_url + 'data/gis/ro_coldep.topojson').done(function (geodata) {
                addTopoData(geodata);
            });
        }
    });
